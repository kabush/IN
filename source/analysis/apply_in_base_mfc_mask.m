...
for i=1:numel(var_names)
    ...
    cmd = ['! 3dcalc -a ',proj.path.analysis.in_base_clust_thresh, ...
    ...
    cmd = ['! 3dcalc -a ',proj.path.code,'tmp/bool_clust_mask_',...
           ...
           'sng_orient_mfc_mask_3x3x3.nii.gz ' ...
           '-expr ''bool(a*b)'' -prefix ',proj.path.code,...
           'tmp/mfc_clust_mask_',affect_name,'_',name];
    disp(cmd)
    eval(cmd)

    %% Generate boolean mask for disjoint of mfc * boolean cluster mask
    cmd = ['! 3dcalc -a ',proj.path.code,'tmp/bool_clust_mask_',...
           affect_name,'_',name,'+tlrc -b ',...
           proj.path.code,'tmp/mfc_clust_mask_',affect_name,'_',name,'+tlrc ' ...
           '-expr ''bool(b-a)'' -prefix ',proj.path.code,...
           'tmp/mfc_disjoint_clust_mask_',affect_name,'_',name];
    disp(cmd)
    eval(cmd)
    ...
    cmd = ['! 3dAFNItoNIFTI ', ...
           proj.path.code,'tmp/mfc_disjoint_clust_mask_',affect_name, ...
           '_',name,'+tlrc'];
    disp(cmd);
    eval(cmd);
    
    cmd = ['! mv ./mfc_disjoint_clust_mask_',affect_name,'_',name,'.nii ',...
           proj.path.analysis.in_base_clust_thresh];
    disp(cmd);
    eval(cmd);
    
    %% Generate mfc restricted maps
    cmd = ['! 3dcalc -a ',proj.path.code,'tmp/mfc_disjoint_clust_mask_',...
           affect_name,'_',name,'+tlrc -b ',...
           proj.path.analysis.in_base_clust_thresh,...
           'clust_map_',affect_name,'_',name,'+tlrc ' ...
           '-expr ''a*b'' -float -prefix ',proj.path.code,...
           'tmp/mfc_disjoint_clust_map_',affect_name,'_',name];
    disp(cmd)
    eval(cmd)

    %% Generate *.nii and move
    cmd = ['! 3dAFNItoNIFTI ', ...
           proj.path.code,'tmp/mfc_disjoint_clust_map_',affect_name, ...
           '_',name,'+tlrc'];
    disp(cmd);
    eval(cmd);
    
    cmd = ['! mv ./mfc_disjoint_clust_map_',affect_name,'_',name,'.nii ',...
           proj.path.analysis.in_base_clust_thresh];
    disp(cmd);
    eval(cmd);
