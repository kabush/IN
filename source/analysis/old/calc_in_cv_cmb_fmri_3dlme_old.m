...
for i = 1:numel(subjs)
    ...
    if(data_exist)
        ...
        evc_indx_box = eval(['mdls.',affect_name,'_indx']); %%evc
                                                        %%limiting factor
        ...
        % use evc indices to limit indices of non-evc ccms
        err_indx_box = reshape(err_indx_box',1,prod(size(err_indx_box)))';
        evc_indx_box = reshape(evc_indx_box',1,prod(size(evc_indx_box)))';
        [indx,indxa,indxb] = intersect(err_indx_box,evc_indx_box);

        err = err(indxa);
        cnf = cnf(indxa);
        cnf_alt = cnf_alt(indxa);
        pel = pel(indxa);
        pro = pro(indxa);
        
        %order of observations
        traj_box = repmat([1:4],numel(indx)/4,1);
