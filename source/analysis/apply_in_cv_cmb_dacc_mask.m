...
function [] = apply_in_cv_cmb_dacc_mask(proj,affect_name)
...
for i=1:numel(var_names)
    ...
    cmd = ['! 3dcalc -a ',proj.path.code,'tmp/bool_clust_mask_',...
           ...
           'tmp/dacc_clust_mask_',affect_name,'_',name];
    ...
    cmd = ['! mv ./dacc_clust_mask_',affect_name,'_',name,'.nii ',...
    ...
    cmd = ['! 3dcalc -a ',proj.path.code,'tmp/dacc_clust_mask_',...
           ...
           'tmp/dacc_clust_map_',affect_name,'_',name];
    disp(cmd)
    eval(cmd)
    ...
    cmd = ['! mv ./dacc_clust_map_',affect_name,'_',name,'.nii ',...
           proj.path.analysis.in_cv_cmb_clust_thresh];
    disp(cmd);
    eval(cmd);
