...
function [] = apply_in_cv_cmb_clust_mfc_mask_2fstat(proj,affect_name)
...
for i=1:numel(var_ids)
    ...
    %% Generate mfc+cluster restricted fstat mask
    cmd = ['! 3dcalc -a ',proj.path.code,'tmp/clust_mask_',...
           ...
           'tmp/mfc_clust_mask_',affect_name,'_',name,'_fstat'];
    disp(cmd)
    eval(cmd)

    %% Generate mfc+cluster restricted fstat *map
    cmd = ['! 3dcalc -a ',proj.path.analysis.in_cv_cmb_3dlme, ...
          'lme_',affect_name,'_cv_cmb+tlrc[',num2str(id),'] -b ',...
          proj.path.code,'tmp/mfc_clust_mask_',affect_name,'_',name,...
          '_fstat+tlrc -expr ''a*b'' -float -prefix ',proj.path.code,...
          'tmp/mfc_clust_map_',affect_name,'_',name,'_fstat'];
    disp(cmd)
    eval(cmd)
    ...
    cmd = ['! mv ./mfc_clust_mask_',affect_name,'_',name,'_fstat.nii ',...
           proj.path.analysis.in_cv_cmb_clust_thresh];
    disp(cmd);
    eval(cmd);
    ...
    cmd = ['! mv ./mfc_clust_map_',affect_name,'_',name,'_fstat.nii ',...
           proj.path.analysis.in_cv_cmb_clust_thresh];
    disp(cmd);
    eval(cmd);
