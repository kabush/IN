...
logger(['Model REST affect dynamics (Platt space) '],proj.path.logfile);
logger(['************************************************'],proj.path.logfile);

%% Set-up Directory Structure for fMRI betas
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.mvpa.fmri_rest_via_ex_gm_mdl]);
    eval(['! rm -rf ',proj.path.mvpa.fmri_rest_via_ex_gm_mdl]);
    disp(['Creating ',proj.path.mvpa.fmri_rest_via_ex_gm_mdl]);
    eval(['! mkdir ',proj.path.mvpa.fmri_rest_via_ex_gm_mdl]);
end
...
for i = 1:numel(subjs)
    ...
    try
        ...
        % Test for completeness of this subject (normal quality
        % check not working)
...
for i = 1:numel(subjs)
    ...
    for j=1:N_iter
        ...
        %% extract subject info
        cv_subj_study = subjs{i}.study;
        cv_subj_id = subjs{i}.id;
        cv_name = subjs{i}.name;
        ...
        cv_v_tilde = 1./(1+exp(-hd(:,2)));
        
        [cls hd] = predict(a_model,all_img);
        cv_a_tilde = 1./(1+exp(-hd(:,2)));
    ...
    % %% Save out data
    save([proj.path.mvpa.fmri_rest_via_ex_gm_mdl,subj_study,'_',name,'_slf_mu_v_rest.mat'],'slf_mu_v_rest');
    save([proj.path.mvpa.fmri_rest_via_ex_gm_mdl,subj_study,'_',name,'_slf_mu_a_rest.mat'],'slf_mu_a_rest');
