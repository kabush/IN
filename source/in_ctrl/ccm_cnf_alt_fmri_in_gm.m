...
logger(['Computing IN Cog Control Conflict Models                  '],proj.path.logfile);
logger(['*************************************************'],proj.path.logfile);
...
for i = 1:numel(subjs)
    ...
    if(data_exist)
        ...
        %% ----------------------------------------
        %% Model Conflict (interaction of valence and arousal)

        % valence
        mdls.v_dcmp = abs(prds.v_dcmp.h(:,3:(end-1))).*abs(prds.a_dcmp.h(:,3:(end-1)));
