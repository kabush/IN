...
%% Use group action to build 5 discrete action partitions
...
act_a_std = std(act_a_srt);

%% compute partition based on standard deviations
act_v_5part = [-2*act_v_std,-act_v_std,act_v_std,2*act_v_std];
act_a_5part = [-2*act_a_std,-act_a_std,act_a_std,2*act_a_std];
...
for i = 1:numel(subjs)
    ...
    if(data_exist)
        ...
        dsc_actions_1d(find(actions_1d>act_v_5part(3)))=1; 
        dsc_actions_1d(find(actions_1d>act_v_5part(4)))=2; 
        dsc_actions_1d(find(actions_1d<act_v_5part(2)))=-1; 
        dsc_actions_1d(find(actions_1d<act_v_5part(1)))=-2; 
        ...
        actions_1d = zscore(reshape(actions',1,prod(size(actions))));
        dsc_actions_1d = 0*actions_1d;
        dsc_actions_1d(find(actions_1d>act_a_5part(3)))=1; 
        dsc_actions_1d(find(actions_1d>act_a_5part(4)))=2; 
        dsc_actions_1d(find(actions_1d<act_a_5part(2)))=-1; 
        dsc_actions_1d(find(actions_1d<act_a_5part(1)))=-2; 
        dsc_act_a_all = [dsc_act_a_all,dsc_actions_1d];
        ...
        errors_1d = zscore(reshape(errors',1,prod(size(errors))));
        dsc_err_a_all = [dsc_err_a_all,errors_1d];
...
grp_act_a_mean = mean(dsc_act_a_all);
grp_act_a_std = std(dsc_act_a_all);
grp_err_a_mean = mean(dsc_err_a_all);
grp_err_a_std = std(dsc_err_a_all);
...
% act_dscr = action_5dscr_set(a); % not needed
...
for i=1:numel(subjs)
    ...
    if(data_exist)
        ...
        %% Rescale actions and errors at the group level
        rwd_actions_1d = (dsc_actions_1d-grp_act_a_mean)/grp_act_a_std;
        rwd_errors_1d = (errors_1d-grp_err_a_mean)/grp_err_a_std;
...
%% ----------------------------------------
%% ----------------------------------------
%% Analyze control across task
%% ----------------------------------------
%% ----------------------------------------

Q_traj_v_all = zeros(Nsbj,30,4);
Q_rand_v_all = zeros(Nsbj,30,4);
act_err_v_all = zeros(Nsbj,30,4);
Q_best_v_all = zeros(Nsbj,30,4);
Q_worst_v_all = zeros(Nsbj,30,4);
...
for i = 1:numel(subjs)
    ...
    try
        ...
        sbj_i = sbj_i+1;
        Q_traj_v_all(sbj_i,:,:) = reshape(Q_traj,4,30)';
        Q_rand_v_all(sbj_i,:,:) = reshape(Q_rand,4,30)';
        Q_best_v_all(sbj_i,:,:) = reshape(Q_best,4,30)';
        Q_worst_v_all(sbj_i,:,:) = reshape(Q_worst,4,30)';
