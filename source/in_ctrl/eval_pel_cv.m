...
function [pel_all_out,pel_all,mdl] = eval_pel_cv(proj,affect_name);
    
subjs = load_subjs(proj);

% log progress
logger('* Fitting the PEL',proj.path.logfile);
...
for i=1:numel(subjs)
    ...
    if(data_exist)
        ...
        % activations)
        ica_seq = proj.param.ctrl.ica_ids; %[1:18]; % *** TICKET: EMOTION ICs ONLY ***
        Nica = numel(ica_seq);
        ...
        %% ------------------------------------------------------------
        %% Store out states
        save([proj.path.ctrl.in_pel_opt_mdl,subj_study,'_',name,'_states.mat'],'states');
        ...
        %% ------------------------------------------------------------
        %% Store out model
        save([proj.path.ctrl.in_pel_opt_mdl,subj_study,'_',name,'_pel_mdl_',affect_name,'.mat'],'mdl');
