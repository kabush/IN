...
if(proj.process.mri)
    ...
    for i=1:numel(subjs)
        ...
        if exist([proj.path.mri.mri_clean,subj_study,'_',name,'/identify/run1/' ...
            ...
            if(subj.mri.id1.exist)
               ...
               ,subj_study,'.',name,'.identify.run1.censor.1D']);
                ...
                %debug
                disp(['****Identify bad motion: ',num2str(round(100*N_bad/N_all)),'%']);
        ...
        %% EPI images
        if(subj.mri.id1.exist & subj.mri.id1.motion_ok)
            subj.mri.id1.ok = 1;
        end

        if(subj.mri.id2.exist & subj.mri.id2.motion_ok)
            subj.mri.id2.ok = 1;
        end
    ...
    disp('No MRI quality check.  Processing not complete.');
end
